...
def search_filenames(
    ...
    csv_names = list(filter(lambda file_name: file_name.endswith(file_end), file_names))

    filtered_names = []
    for s_filter in search_filters:
        filtered_names += list(
            filter(lambda file_name: s_filter in file_name, csv_names)
        )
    ...
    sankey_dict.update(
        {"all_files_in": len(csv_names), "filtered_files_in": len(filtered_names)}
    )
...
def get_df_folder(
    folder_path,
    file_names,
    index_columns=["cell_ID", "EIS_measurement_id", "EIS_Frequency"],
):
    """
...
def add_cartesian(df, enable_degree=False):
    ...
    df.insert(
        df.columns.get_loc("EIS_Z_phase") + 1, "EIS_Z_Re", np.real(impedance_z), True
    )
    df.insert(
        df.columns.get_loc("EIS_Z_Re") + 1, "EIS_Z_Im", np.imag(impedance_z), True
    )
...
def plot_frequency_groups(fig, ax1, ax2, frequency_groups, df):
    ...
    frequencies = (
        df.index.droplevel("cell_ID").droplevel("EIS_measurement_id").to_numpy()
    )
    ...
    boxprops = dict(linewidth=1.5, color=eisplot.rwth_colors.colors[("black", 100)])
    ...
    ax2.scatter(
        0,
        np.mean(frequencies_log),
        s=10,
        color=eisplot.rwth_colors.colors[("red", 100)],
        zorder=3,
    )
...
class FrequencySelectWidget:
    ...
    def __init__(self, axs, fig, df, frequency_groups_df, frequency_groups):
        ...
        self.freq_list_group_string = [
            ("%d" % item) for item in frequency_groups_df.index
        ]
        self.freq_list_occurrence_string = [
            ("%0.6f" % item)
            for item in frequency_groups_df.loc[:, "occurrence"]
            / np.sum(frequency_groups_df.loc[:, "occurrence"])
        ]
        self.freq_list_length_string = [
            ("%02d" % item) for item in frequency_groups_df.loc[:, "length"]
        ]
        self.freq_list_min_string = [
            ("%5.4f" % item) for item in frequency_groups_df.loc[:, "min"]
        ]
        self.freq_list_max_string = [
            ("%5.4f" % item) for item in frequency_groups_df.loc[:, "max"]
        ]

        self.freq_list = np.empty([1, len(self.freq_list_occurrence_string)], dtype="U")

        self.string_tmp = np.repeat("Group: ", len(self.freq_list))
        self.freq_list = np.core.defchararray.add(self.freq_list, self.string_tmp)
        ...
        self.slider_min = widgets.FloatLogSlider(
            min=np.max(
                [
                    -2,
                    np.floor(np.log10(np.min(pd.unique(df.index.get_level_values(2))))),
                ]
            ),
            max=np.ceil(np.log10(np.max(pd.unique(df.index.get_level_values(2))))),
            value=np.log10(
        ...
        self.slider_max = widgets.FloatLogSlider(
            value=np.min([4500, np.max(pd.unique(df.index.get_level_values(2)))]),
            min=np.floor(np.log10(np.min(pd.unique(df.index.get_level_values(2))))),
            max=np.ceil(np.log10(np.max(pd.unique(df.index.get_level_values(2))))),
            base=10,
        ...
        self.coverage = widgets.FloatProgress(
            ...
            bar_style="info",
            style={
                "bar_color": eisplot.rwth_colors.colors[("blue", 100)],
        ...
        self.f_menu_f_manual = widgets.VBox([self.slider, self.slider_f_per_decade])
        ...
        self.out = widgets.interactive_output(
            self.update_f,
            {
                ...
                "fig": fixed(fig),
                "df": fixed(df),
                "frequency_groups": fixed(frequency_groups),
                "f_menu_sel_p": self.f_menu_sel,
                "slider_min_p": self.slider_min,
                "slider_max_p": self.slider_max,
    ...
    def plot_hist_freq(
        self, all_frequencies, selected_frequencies, ax, label="", alpha=1.0
    ):
        """
        ...
        ax.bar(center, hist, width=np.diff(bin_edges) * 0.8, label=label, alpha=alpha)
    ...
    def update_coverage(self, percentage):
        ...
        """
        self.coverage.description = (
            "Remaining Frequencies: " + ("%02.2f" % percentage) + " %"
        )
    ...
    def cal_coverage(self, f_min, f_max, frequency_groups_df):
        ...
        covered_count = np.sum(
            (
                (frequency_groups_df["min"] <= f_min)
                & (frequency_groups_df["max"] >= f_max)
            )
            * frequency_groups_df.loc[:, "occurrence"]
        )
    ...
    def update_f(
        ...
        df,
        frequency_groups,
        f_menu_sel_p="Frequency Group",
        slider_min_p=0.1,
        slider_max_p=1000,
        dropdown_p=0,
        slider_f_per_decade_p=6,
    ):
        """
        ...
        if f_menu_sel_p == "Frequency Group":
            ...
            self.update_coverage(
                self.cal_coverage(
                    np.min(frequency_groups[self.dropdown.value]),
                    np.max(frequency_groups[self.dropdown.value]),
                    self.frequency_groups_df,
                )
            )
            ...
            self.frequencies = np.logspace(
                np.log10(self.slider_min.value),
                np.log10(self.slider_max.value),
                round(
                    np.ceil(
                        (
                            np.log10(self.slider_max.value)
                    ...
                    * self.slider_f_per_decade.value
                ),
            )
            self.update_coverage(
                self.cal_coverage(slider_min_p, slider_max_p, self.frequency_groups_df)
            )
...
class LinkkSelectWidget:
    ...
    def __init__(self, ax1, ax2, fig, df):
        ...
        self.plot_linKK(
            np.median(df.groupby(level=[0, 1])["linKK"].mean().to_numpy()),
            df,
            ax1,
            alpha=1.0,
        )
        ...
        self.out = widgets.interactive_output(
            self.update_linKK,
            {
                ...
                "fig": fixed(fig),
                "df": fixed(df),
                "slider_linKK_p": self.slider_linKK,
            },
        )
        display(self.ui, self.out)
    ...
    def update_coverage_linKK(self, percentage):
        ...
        """
        self.coverage_linKK.description = (
            "Remaining Measurements: " + ("%02.2f" % percentage) + " %"
        )
    ...
    def cal_coverage_linKK(self, linKK_limit, df):
        ...
        """
        total_count = np.sum(df.groupby(level=[0, 1])["linKK"].mean().to_numpy() >= 0)
        covered_count = np.sum(
            df.groupby(level=[0, 1])["linKK"].mean().to_numpy() <= linKK_limit
        )
        return covered_count / total_count * 100
    ...
    def plot_linKK(self, linKK_limit, df, ax, alpha=1.0):
        ...
        ax.scatter(
            ...
            np.sort(df.groupby(level=[0, 1])["linKK"].mean().to_numpy()),
    ...
    def plot_linKK_stats(self, linKK_limit, df, ax, alpha=1.0):
        ...
        violins = ax.violinplot(
            y_data, positions=[0], bw_method="silverman", showextrema=False, points=5000
        )
        ...
        boxprops = dict(
            linewidth=1.5, color=eisplot.rwth_colors.colors[("petrol", 100)]
        )
        ...
        ax.plot(
            ax.get_xlim(),
            np.ones([len(ax.get_xlim()), 1]) * linKK_limit,
            label="Limit",
            color=eisplot.rwth_colors.colors[("black", 100)],
            alpha=alpha,
        )
    ...
    def update_linKK(self, ax1, ax2, fig, df, slider_linKK_p=0.1):
        ...
        ax1.cla()
        self.update_coverage_linKK(self.cal_coverage_linKK(self.slider_linKK.value, df))
...
def filter_shapes(df, frequencies):
    ...
    drop_list = [splitted_indexes[drop_index] for drop_index in drop_measurements]
...
def restruct_df(
    df,
    frequencies,
    eis_column_names=["EIS_Z_abs", "EIS_Z_phase"],
    ...
    average_keys = list(filter(lambda key: key not in eis_column_names, all_keys))
    ...
    for freq_count, freq in enumerate(frequencies):
        ...
        df_freq = df.loc[np.isclose(index_freqs, freq, rtol=relative_tolerance)]
        ...
        for av_count, av_key in enumerate(average_keys[1:]):
            if pd.to_numeric(df_freq[av_key], errors="coerce").notnull().all():
                average_params[av_count, :, freq_count] = df_freq[av_key].to_numpy()
            else:
                average_params[av_count, :, freq_count] = df_freq[av_key].values[0]
...
def get_key_lookup_df(eis_column_names, frequencies):
    ...
    """
    new_data = np.empty([len(frequencies), len(eis_column_names)], dtype="<U1000")

    for freq_ind, freq in enumerate(frequencies):
        for eis_col_ind, eis_column_name in enumerate(eis_column_names):
            new_data[freq_ind, eis_col_ind] = get_column_key(eis_column_name, freq)

    key_lookup_df = pd.DataFrame(new_data, index=frequencies, columns=eis_column_names)
    key_lookup_df.index.name = "frequency"

    return key_lookup_df


def add_ECMfit_parallel_func(
    df, frequencies, abs_keys, phase_keys, c_circuit, c, global_fit=False, maxfev=4000
):
    """
    ...
    for cell_ID_loop in df.index.unique("cell_ID"):
        for EIS_measurement_id_loop in df.loc[cell_ID_loop].index.unique(
            "EIS_measurement_id"
        ):
            if len(frequencies) > len(c) + 2:  # bad error handling
                abs_values = (
                    df.loc[cell_ID_loop]
                    .loc[EIS_measurement_id_loop][abs_keys]
                    .to_numpy(dtype="float64")
                )  # important to use dtype='float64'
                phases = (
                    df.loc[cell_ID_loop]
                    .loc[EIS_measurement_id_loop][phase_keys]
                    .to_numpy(dtype="float64")
                )  # important to use dtype='float64'
                impedance_z = abs_values * np.exp(1j * phases)
...
def add_linKK_parallel_func(
    df, linKK_cutoff_mu=0.85, linKK_max_M=100, residuals_quantile=1
):
    """
    ...
    for cell_ID_loop in df.index.unique("cell_ID"):
        for EIS_measurement_id_loop in df.loc[cell_ID_loop].index.unique(
            ...
            frequency = (
                df.loc[cell_ID_loop].loc[EIS_measurement_id_loop].index.to_numpy()
            )
            abs_values = (
                df.loc[cell_ID_loop]
                .loc[EIS_measurement_id_loop]
                .loc[:, "EIS_Z_abs"]
                .to_numpy(dtype="float64")
            )
            phases = (
...
def add_linKK(df, linKK_cutoff_mu=0.85, linKK_max_M=100, residuals_quantile=1):
    ...
    df_list = [
        df_new_index.loc[cell_names_list[i]].set_index(["EIS_Frequency"], append=True)
    ...
    results = pool.starmap(
        add_linKK_parallel_func,
        zip(df_list, linKK_cutoff_mu_list, linKK_max_M_list, residuals_quantile_list),
    )
    df = pd.concat(results)
    pool.close()
...
def get_frequency_groups_parallel_func(df):
    ...
    df_fs = df.index
    df_fs = pd.DataFrame(
        df_fs.tolist(), columns=["cell_ID", "EIS_measurement_id", "EIS_Frequency"]
    )
    ...
    for cell_index, cell_name in enumerate(all_cell_ids):
        ...
        for m_id_index, m_id in enumerate(all_measurements):
            f_list_of_this_m = (
                df_fs.loc[df_fs.cell_ID == cell_name]
                .loc[df_fs.EIS_measurement_id == m_id]
                .EIS_Frequency.to_list()
            )
            if f_list_of_this_m in frequency_groups:
                freq_group_ind = frequency_groups.index(f_list_of_this_m)
                frequency_groups_occurrence[freq_group_ind] = (
                    frequency_groups_occurrence[freq_group_ind] + 1
                )
            else:
    ...
    frequency_groups_df = pd.DataFrame(
        np.transpose(
            [
                frequency_groups_occurrence,
                frequency_groups_length,
                frequency_groups_min,
                frequency_groups_max,
            ]
        ),
        columns=["occurrence", "length", "min", "max"],
    )
...
def get_frequency_groups(df):
    ...
    frequency_groups_df_split = frequency_groups_df_split.reset_index(drop=True)
    ...
    for freq_group_ind_split, freq_group in enumerate(frequency_groups_split):
        if freq_group in frequency_groups:
            ...
            frequency_groups_occurrence[freq_group_ind_new] += (
...
def filer_by_frequencies_groups_parallel_func(
    df, frequencies, minimal_amount_of_frequencies
):
    """
    ...
    for cell_ID_loop in df.index.unique("cell_ID"):
        for EIS_measurement_id_loop in df.loc[cell_ID_loop].index.unique(
            ...
            if (
                np.max(df.loc[cell_ID_loop].loc[EIS_measurement_id_loop].index)
                < np.max(frequencies)
            ) or (
                np.min(df.loc[cell_ID_loop].loc[EIS_measurement_id_loop].index)
                > np.min(frequencies)
            ):
                print(
                    "Dropped Cell: "
                    + cell_ID_loop
                    + ", Measurement: "
                    + str(EIS_measurement_id_loop)
                )
                sys.stdout.flush()
                df = (
                    df.reset_index(level="EIS_Frequency")
                    .drop(index=(cell_ID_loop, EIS_measurement_id_loop))
                    .set_index(["EIS_Frequency"], append=True)
                )
            elif (
                len(df.loc[cell_ID_loop].loc[EIS_measurement_id_loop].index)
                < minimal_amount_of_frequencies
            ):
                print(
                    "Dropped Cell: "
                    + cell_ID_loop
...
def filter_by_frequencies(df, frequencies, minimal_amount_of_frequencies):
    ...
    measurements_before_delte = len(df.index.droplevel("EIS_Frequency").unique())
    ...
    results = pool.starmap(
        filer_by_frequencies_groups_parallel_func,
        zip(df_list, frequencies_list, minimal_amount_of_frequencies_list),
    )
    df = pd.concat(results)
    pool.close()

    measurements_after_delte = len(df.index.droplevel("EIS_Frequency").unique())
...
def eis_interpolate_restruct_parallel_func(df, new_frequencies):
    ...
    average_keys = list(filter(lambda key: key not in eis_column_names, all_keys))
    # set column names for new DataFrame
    eis_columns = average_keys + get_eis_columns(eis_column_names, new_frequencies)
    # select individual measurements as index for new DataFrame
    measure_ids = df.index.droplevel(2)
    measure_ids = measure_ids[np.invert(measure_ids.duplicated(keep="first"))]
    # create empty new Dataframe
    ...
    for cell_ID_loop in df.index.unique("cell_ID"):
        for EIS_measurement_id_loop in df.loc[cell_ID_loop].index.unique(
            ...
            z_abs = (
                ...
                .loc[:, "EIS_Z_abs"]
                .to_numpy(dtype="float64")
            )  # very important to use dtype='float64'!!!
            if "EIS_Z_phase" in df.columns:
                z_phase = (
                    df.loc[cell_ID_loop]
                    .loc[EIS_measurement_id_loop]
            ...
            if "EIS_Z_phase" in df.columns:
                ...
                z_dict = dict(
                    zip(
                        eis_column_names,
                        [z_abs_inter, z_phase_inter, z_real_inter, z_imag_inter],
                    )
                )
            else:
                # critical to keep order correct!!!!
            ...
            for f_index, f in enumerate(new_frequencies):
                for eis_key_index, eis_key in enumerate(eis_column_names):
                    ...
                    df_new.loc[(cell_ID_loop, EIS_measurement_id_loop), column_key] = (
                        z_dict[eis_column_names[eis_key_index]][f_index]
                    )
            ...
            for average_key_index, average_key in enumerate(average_keys):
                if (
                    ...
                    df_new.loc[(cell_ID_loop, EIS_measurement_id_loop), average_key] = (
                        value
                    )
                else:
                    if average_key == "Time":
                        ...
                        df_new.loc[
                            (cell_ID_loop, EIS_measurement_id_loop), average_key
                        ] = value
                    else:
                        value = (
                            df.loc[cell_ID_loop]
...
def eis_interpolate_restruct(df, new_frequencies):
    ...
    results = pool.starmap(
        eis_interpolate_restruct_parallel_func, zip(df_list, new_frequencies_list)
    )
    df = pd.concat(results)
    pool.close()
    return df
...
def add_DRT_parallel_func(
    ...
    for ind, dic in enumerate(DRT_dict):
        DRT_dict[ind] = [
            "DRT_Peak_" + str(dic) + "_tau",
            "DRT_Peak_" + str(dic) + "_gamma",
        ]
    ...
    for cell_ID_loop in df.index.unique("cell_ID"):
        for EIS_measurement_id_loop in df.loc[cell_ID_loop].index.unique(
            ...
            if len(frequencies) > 5:
                ...
                abs_values = df.loc[mask, abs_keys].to_numpy(dtype="float64")[0]
                phase_values = df.loc[mask, phase_keys].to_numpy(dtype="float64")[0]
                impedance = abs_values * np.exp(1j * phase_values)

                N_freqs = len(frequencies)
                freq_min = np.min(frequencies)
                freq_max = np.max(frequencies)
                freq_vec = np.logspace(
                    np.log10(freq_min), np.log10(freq_max), num=N_freqs, endpoint=True
                )
                tau_vec = np.logspace(
                ...
                for ind, value in enumerate(DRT_dict):
                    if ind >= len(peaks):
                        ...
                        df.loc[mask, DRT_dict[value][1]] = gamma_RR_GCV[peaks][ind]
...
def add_DRT(
    ...
    df_list = list(df.loc[cell_names_list[i]] for i in range(len(cell_names_list)))
    ...
    if drt_plot:
        ...
        results = pool.starmap(
            add_DRT_parallel_func, zip(df_list, key_lookup_df_list, drt_plot_list)
        )
        df = pd.concat(results)
        pool.close()
                ...
                for ind, value in enumerate(EIS_Extrema_dict):
                    if ind >= len(nyquist_zero):
                        ...
                        df.loc[mask, EIS_Extrema_dict[value][0:4]] = [
                            frequencies[nyquist_zero[ind]],
                            real_values[nyquist_zero[ind]],
                            frequencies[nyquist_zero[ind]],
                            imag_values[nyquist_zero[ind]],
                        ]
                    ...
                    if ind >= len(nyquist_min):
                        ...
                        df.loc[mask, EIS_Extrema_dict[value][4:8]] = [
                            frequencies[nyquist_min[ind]],
                            real_values[nyquist_min[ind]],
                            frequencies[nyquist_min[ind]],
                            imag_values[nyquist_min[ind]],
                        ]
                    ...
                    if ind >= len(nyquist_max):
                        ...
                        df.loc[mask, EIS_Extrema_dict[value][8:12]] = [
                            frequencies[nyquist_max[ind]],
                            real_values[nyquist_max[ind]],
                            frequencies[nyquist_max[ind]],
                            imag_values[nyquist_max[ind]],
                        ]
                    ...
                    if ind >= len(bode_abs_min):
                        ...
                        df.loc[mask, EIS_Extrema_dict[value][12:14]] = [
                            frequencies[bode_abs_min[ind]],
                            abs_values[bode_abs_min[ind]],
                        ]
                    ...
                    if ind >= len(bode_abs_max):
                        ...
                        df.loc[mask, EIS_Extrema_dict[value][14:16]] = [
                            frequencies[bode_abs_max[ind]],
                            abs_values[bode_abs_max[ind]],
                        ]
                    ...
                    if ind >= len(bode_phase_min):
                        ...
                        df.loc[mask, EIS_Extrema_dict[value][16:18]] = [
                            frequencies[bode_phase_min[ind]],
                            phase_values[bode_phase_min[ind]],
                        ]
                    ...
                    if ind >= len(bode_phase_max):
                        ...
                        df.loc[mask, EIS_Extrema_dict[value][18:20]] = [
                            frequencies[bode_phase_max[ind]],
                            phase_values[bode_phase_max[ind]],
                        ]
...
def add_EIS_Extrema(df, key_lookup_df):
    ...
    results = pool.starmap(
        add_EIS_Extrema_parallel_func, zip(df_list, key_lookup_df_list)
    )
    df = pd.concat(results)
    pool.close()
    return df
